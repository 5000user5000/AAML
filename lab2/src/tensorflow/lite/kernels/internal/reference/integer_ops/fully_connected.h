...
inline void FullyConnected(
    ...
    const int32_t output_offset = 32; // params.output_offset;
    const int32_t output_multiplier = 1633527722; // params.output_multiplier;
    const int output_shift = -8; // params.output_shift;
    ...
    //   const int output_dim_count = 2; // output_shape.DimensionsCount();
    const int batches = 1 ; // FlatSizeSkipDim(output_shape, output_dim_count - 1);
    const int output_depth = 12; // output_shape.Dims(output_dim_count - 1);
    TFLITE_DCHECK_LE(output_depth, filter_shape.Dims(filter_dim_count - 2));
    const int accum_depth = 300; // filter_shape.Dims(filter_dim_count - 1);
    int32_t sum_filter[] = {-634624, -2048, -140416, -96512, -154752, -105728, -101376, -109056, -137216, -142080, -111104, -77184};
    ...
    for (int b = 0; b < batches; ++b) {
        ...
        for (int d = 0; d < accum_depth; d += 4)  {
            uint32_t input_combined = *((uint32_t*)(input_data + b*accum_depth + d)); 
            uint32_t filter_combined = *((uint32_t*)(filter_data + out_c * accum_depth + d)); 
            acc = cfu_op2(0, input_combined, filter_combined);
        }
        acc += sum_filter[out_c]; ;
